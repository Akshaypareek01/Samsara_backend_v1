...
export const getTeacherAvailabilities = async (req, res) => {
  try {
    ...
    const availabilities = await TeacherAvailability.find(query).sort({ date: 1, startTime: 1 });
...
export const deleteAvailability = async (req, res) => {
  try {
    ...
    const availability = await TeacherAvailability.findByIdAndUpdate(id, { isActive: false }, { new: true });
...
export const getAvailableSlots = async (req, res) => {
  try {
    ...
    const availableSlots = await TeacherAvailability.find(query).sort({ startTime: 1 });
