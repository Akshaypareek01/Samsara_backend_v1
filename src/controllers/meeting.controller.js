...
export const createMeeting = async (req, res) => {
  try {
    const { meetingName, title, duration, meetingId, meetingPassword, hostName, teacherName } = req.body;
...
export const getMeetingData = async (req, res) => {
  try {
    ...
    const meeting = meetingId ? await Meeting.findOne({ meetingId }) : await Meeting.find();
