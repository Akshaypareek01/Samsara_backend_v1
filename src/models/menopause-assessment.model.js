...
menopauseAssessmentSchema.pre('save', function (next) {
  ...
  this.totalScore =
    this.scores.irregularPeriodsScore +
    this.scores.fatigueScore +
    this.scores.weightChangesScore +
    this.scores.sleepQualityScore +
...
menopauseAssessmentSchema.methods.calculateRiskLevel = function (averageScore) {
  ...
  if (averageScore >= 2.5) {
    riskLevel = 'Low Risk';
    riskDescription =
      'Your symptoms indicate a low risk of menopause-related issues. Continue maintaining a healthy lifestyle.';
  } else if (averageScore >= 1.5) {
    riskLevel = 'Low-Moderate Risk';
    riskDescription =
      'You may be experiencing some menopause-related symptoms. Consider lifestyle adjustments and consult with healthcare providers if symptoms persist.';
  } else if (averageScore >= 0.5) {
    riskLevel = 'Moderate Risk';
    riskDescription =
      'You are showing moderate signs of menopause-related symptoms. It is recommended to consult with a healthcare provider for proper guidance and management.';
  } else {
    riskLevel = 'High Risk';
    riskDescription =
      'Your symptoms indicate a high risk of menopause-related issues. Immediate consultation with a healthcare provider is strongly recommended for proper assessment and treatment.';
  }
...
menopauseAssessmentSchema.statics.markPreviousAsNotLatest = function (userId) {
  return this.updateMany({ userId, isLatest: true }, { isLatest: false });
};
